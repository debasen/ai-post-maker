...
- `--sleep-<N>`: (Optional) The sleep interval in seconds between runs. Defaults to `600` (e.g. `--sleep-500`).
...
2. **Create or Resume**:
   ...
   - If no existing task list is found, **create** a new Antigravity task list artifact based on the parameters provided. The artifact should alternate between running the `/grok_automation_v4` workflow and waiting for a specified interval (using the value from `--sleep-<N>` if provided, otherwise default `sleep 600`).
...
5. **Wait**: For wait tasks, run the `sleep <interval>` command (using the provided or default value) in the background and use `command_status` to wait for its completion.
